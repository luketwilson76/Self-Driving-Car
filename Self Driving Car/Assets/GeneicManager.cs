...
public class GeneicManager : MonoBehaviour
{
    ...
    {
        //7 needs to be a user defined variable
        //We are selecting a few weights to mutate, not all
        int randomPoints = Random.Range(1, (A.RowCount * A.ColumnCount) / 7);
        Matrix<float> C = A;
        // instead of changing weights randomly, we're incrementing/decrementing weight values
    ...
    private NeuralNetwork[] PickBestPopulation()
    {
        ...
        {
            //we are taking best networks to the new population untouched
            newPopulation[naturallySelected] = population[i].IntialiseCopy(controller.layers, controller.neurons);
